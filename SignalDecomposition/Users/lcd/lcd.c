...
{										   
	...
	HAL_Delay(1);		  // 使用HAL库延时，1ms代替5us（HAL_Delay最小单位是1ms）
...
{ 	
  ...
  Timing.DataSetupTime = 24;     // 数据保存时间
  ...
  // 写时序
	ExtTiming.AddressSetupTime = 8;  // 地址建立时间
  ExtTiming.AddressHoldTime = 0;   // 地址保持时间
  ExtTiming.DataSetupTime = 8;     // 数据保存时间

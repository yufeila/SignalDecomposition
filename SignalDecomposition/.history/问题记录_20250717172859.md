...
然后，我尝试了你上次修改之后的代码，在示波器上，A和A'的现象是这样的：以A为示波器触发源(A是信号源产生的信号)，A'是DDS模块ad9833产生的信号，漂移现象还是存在，但是，A'现在相对A主要是向右漂移，而且仿佛在左侧有一根线在拽着A', 使A'会现在一个平衡位置附近小幅震动，然后仿佛“挣断绳子”一样，往右侧突然漂移一段距离。

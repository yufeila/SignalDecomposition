...
/**
 * @brief Removes DC bias from ADC raw data and converts to voltage.
 *
 * This function calculates the mean value of the input ADC samples,
 * subtracts the mean from each sample to remove DC bias, and then
 * converts the result to voltage using ADC_LSB_VOLT.
 *
 * @param[in]  adc_raw     Pointer to the array of raw ADC data (uint16_t).
 * @param[out] adc_zeroed  Pointer to the array for zero-biased, voltage-converted data (float).
 * @param[in]  len         Number of samples in the input array.
 */

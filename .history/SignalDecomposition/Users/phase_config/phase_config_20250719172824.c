...
{
    printf("receive phase_config: phi_deg: %f, freq_Hz_A: %f, freq_Hz_B: %f\r\n", cfg->phi_deg, cfg->freq_Hz_A, cfg->freq_Hz_B);
    // --- 电容参数 ---
    ...
    for(int i = -2; i <= 2; ++i) {
        for(int j = -2; j <= 2; ++j) {
            ...
            float phi = 2.0f * phi_A * (cfg->freq_Hz_B/cfg->freq_Hz_A);
            float err = fabsf(phi_A * 180.0f / PI - cfg->phi_deg);
            if (err < best_err) {
                best_err = err;
                best1 = t1; best2 = t2;
    ...
    printf("R1_ohm: %f, R2_ohm: %f, phi_actual_deg: %f, error_deg: %f\r\n", cfg->R1_ohm, cfg->R2_ohm, cfg->phi_actual_deg, cfg->error_deg);
    printf("\r\n");

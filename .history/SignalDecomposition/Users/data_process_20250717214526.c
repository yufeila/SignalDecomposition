...
void Calibration_Frequency(void)
{
    ...
    /* ---- 通道 A：改进的FLL控制器 ---- */
	/* 死区处理 */
	float dfA_processed = (fabsf(dfA) < FREQ_DEAD_ZONE) ? 0.0f : dfA;
	
	/* PI控制器 - 按O3建议的并行Anti-wind-up结构 */
	float u_A = KP * dfA_processed + pllA_int;  // PI输出
	
	/* 统一限幅 */
	float u_A_sat = (u_A > MAX_FREQ_STEP) ? MAX_FREQ_STEP : 
	                (u_A < -MAX_FREQ_STEP) ? -MAX_FREQ_STEP : u_A;
	
	/* Anti-wind-up：积分器包含限幅反馈 */
	pllA_int += KI * dfA_processed + K_AW * (u_A_sat - u_A);
	
	/* 积分器自身限幅（防止极端情况） */
	float int_limit = MAX_FREQ_STEP * 2.0f;  // 积分限幅为步进限幅的2倍
	if (pllA_int > int_limit) pllA_int = int_limit;
	if (pllA_int < -int_limit) pllA_int = -int_limit;

    /* 应用频率调整 */
    if(fabsf(dfA) > TOL_HZ) {
        int32_t dFTW = (int32_t)(u_A_sat * FTW_SCALE);
        FTW1_cur += dFTW;
        AD9833_WriteFTW1(FTW1_cur);
        printf("Channel A: dfA=%.3f, u_A=%.3f, u_A_sat=%.3f, dFTW=%d\r\n", 
               dfA, u_A, u_A_sat, dFTW);
    }

	/* ---- 通道 B：同样的改进FLL控制器 ---- */
	/* 死区处理 */
	float dfB_processed = (fabsf(dfB) < FREQ_DEAD_ZONE) ? 0.0f : dfB;
	
	/* PI控制器 */
	float u_B = KP * dfB_processed + pllB_int;
	
	/* 统一限幅 */
	float u_B_sat = (u_B > MAX_FREQ_STEP) ? MAX_FREQ_STEP : 
	                (u_B < -MAX_FREQ_STEP) ? -MAX_FREQ_STEP : u_B;
	
	/* Anti-wind-up */
	pllB_int += KI * dfB_processed + K_AW * (u_B_sat - u_B);
	
	/* 积分器限幅 */
	if (pllB_int > int_limit) pllB_int = int_limit;
	if (pllB_int < -int_limit) pllB_int = -int_limit;

    /* 应用频率调整 */
	if (fabsf(dfB) > TOL_HZ) {
		int32_t dFTW = (int32_t)(u_B_sat * FTW_SCALE);
		FTW2_cur += dFTW;
		AD9833_WriteFTW2(FTW2_cur);
        printf("Channel B: dfB=%.3f, u_B=%.3f, u_B_sat=%.3f, dFTW=%d\r\n", 
               dfB, u_B, u_B_sat, dFTW);
	}

	/* 控制器状态监控 */
	printf("Controller states: pllA_int=%.3f, pllB_int=%.3f\r\n", pllA_int, pllB_int);

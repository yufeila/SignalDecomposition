...
void Calibration_Frequency(void)
{
    ...
    static float buf_Bp[CALIBRATION_BUF_SIZE];
    DemuxADCData(calibration_buffer_A, buf_A, buf_Ap, CALIBRATION_BUF_SIZE);
    DemuxADCData(calibration_buffer_B, buf_B, buf_Bp, CALIBRATION_BUF_SIZE);

    // 6. 零交检测
    float *zc_idx_A = NULL;
    float *zc_idx_Apr = NULL;
    float *zc_idx_B = NULL;
    float *zc_idx_Bpr = NULL;

    uint16_t zc_count_A = zero_cross_detection(buf1_A, CALIBRATION_BUF_SIZE, &zc_idx_A);
    uint16_t zc_count_Apr = zero_cross_detection(buf2_A, CALIBRATION_BUF_SIZE, &zc_idx_Apr);
    uint16_t zc_count_B = zero_cross_detection(buf1_B, CALIBRATION_BUF_SIZE, &zc_idx_B);
    uint16_t zc_count_Bpr = zero_cross_detection(buf2_B, CALIBRATION_BUF_SIZE, &zc_idx_Bpr);
...
// 零交检测函数
// 输入：x - 输入float数组, N - 数组长度
// 输出：zc_idx - 用于存储零交分数索引的位置（动态分配）, 返回零交点数量
uint16_t zero_cross_detection(float *x, uint16_t N, float **zc_idx)
{
    uint16_t max_zc = N - 1;
    float *idx_buf = (float *)malloc(max_zc * sizeof(float));
    uint16_t count = 0;

    for (uint16_t i = 0; i < N - 1; ++i) {
        if (x[i] < 0 && x[i+1] >= 0) { // 从负到正
            float frac = x[i] / (x[i] - x[i+1]); // 线性插值
            idx_buf[count++] = i + frac; // 分数采样点
        }
    }
    // 返回数组
    *zc_idx = (float *)malloc(count * sizeof(float));

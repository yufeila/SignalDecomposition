...
void Calibration_Frequency(void)
{
    ...
    // 5. 处理本轮采集的数据
    static float buf1[CALIBRATION_BUF_SIZE];
    static float buf2[CALIBRATION_BUF_SIZE];
    DemuxADCData(calibration_buffer_A, buf1, buf2, CALIBRATION_BUF_SIZE);
    DemuxADCData(calibration_buffer_B, buf1, buf2, CALIBRATION_BUF_SIZE);

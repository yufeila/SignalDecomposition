...
#define MAVG                    20          /* 用 50 个周期做平均，增加频率测量稳定性 */
#define MAX_ZC                  64          /* 最大零交点检测数 (增加以容纳更长MAVG) */

/* FLL (锁频环) 控制器设计参数 */
#define FLL_BANDWIDTH_HZ        15.0f   /* 环路带宽(Hz): 决定响应速度，值越小越稳定，但响应越慢 (建议范围 5-50) */
#define FLL_DAMPING_RATIO       0.9f    /* 阻尼比: 决定超调量，0.707是临界阻尼，>0.7会更稳定 (建议范围 0.7-1.0) */
#define FLL_MAX_STEP_HZ         15.0f   /* 单次最大调节步进(Hz): 限制输出变化率 */
#define FLL_DEAD_ZONE_HZ        2.0f    /* 死区(Hz): 忽略此范围内的频率误差，防止追踪噪声 */

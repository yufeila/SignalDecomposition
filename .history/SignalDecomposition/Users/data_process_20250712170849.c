...
do {                                                                               \
    goertzel_cfg_fω_t g1, g3, g5;                                                  \
    float mag1, ph1, mag3, ph3, mag5, ph5;                                         \
                                                                                   \
    /* -------- 一次性把 1×/3×/5× 幅值算出来 -------- */                          \
    goertzel_init_fω(&g1, N_RAW, 1.0f * (_f_base),  FS_HZ);                        \
    goertzel_init_fω(&g3, N_RAW, 3.0f * (_f_base),  FS_HZ);                        \
    goertzel_init_fω(&g5, N_RAW, 5.0f * (_f_base),  FS_HZ);                        \

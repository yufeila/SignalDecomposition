...
void Calibration_Frequency(void)
{
    ...
    uint16_t zc_count_A = zero_cross_detection(buf_A, CALIBRATION_BUF_SIZE, &zc_idx_A);
    uint16_t zc_count_Apr = zero_cross_detection(buf_Apr, CALIBRATION_BUF_SIZE, &zc_idx_Apr);
    uint16_t zc_count_B = zero_cross_detection(buf_B, CALIBRATION_BUF_SIZE, &zc_idx_B);
    uint16_t zc_count_Bpr = zero_cross_detection(buf_Bpr, CALIBRATION_BUF_SIZE, &zc_idx_Bpr);
